scene.onHitWall(SpriteKind.Enemy, function on_hit_wall(sprite: Sprite, location: tiles.Location) {
    if (tiles.tileIs(tiles.locationOfSprite(sprite), assets.tile`
            derecha
            `)) {
        sprite.vy = 0
        sprite.vx = enemigo_velocidad
    } else if (tiles.tileIs(tiles.locationOfSprite(sprite), assets.tile`
            abajo1
            `)) {
...
game.onUpdateInterval(500, function on_update_interval() {
    ...
    nuevo_enemigo = sprites.create(assets.image`
        caca
        `, SpriteKind.Enemy)
